...
| Pronóstico 7 días y probabilidad de lluvia | Open-Meteo (modelos numéricos, probabilidad por ensamble). Si Open-Meteo rechaza al servidor público (IP compartida), se usa la copia horaria que baja **GitHub Actions** (`.github/workflows/datos-clima.yml` → rama `datos`). Último respaldo: MET Norway (sin probabilidad: en ese caso no se dan avisos de lluvia) | Sí | cada 60 min | No | Modelo: se rotula como tal. Siempre hay enlace al pronóstico oficial del SMN de la localidad |
| Pronóstico oficial por localidad con probabilidad | SMN (smn.gob.ar/pronostico) | **No automatizable**: el SMN protege su servicio con Cloudflare y bloquea el acceso automatizado (verificado 24/09/2026). Se enlaza en cada localidad | — | — | Oficial |
